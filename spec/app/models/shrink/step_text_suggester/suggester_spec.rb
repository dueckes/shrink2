describe Shrink::StepTextSuggester::Suggester do

  context "#suggestions_for" do

    before(:each) do
      @context = mock("Context", :null_object => true)
      Shrink::StepTextSuggester::Context.stub!(:new).and_return(@context)
      Shrink::StepTextSuggester::Suggester::SUGGESTER_CHAIN.each { |suggester| suggester.stub!(:suggestions_for) }
      @text = "Some Text"
      @position = 0
    ...
    it "should create a context from the provided text, position and scenario" do
      Shrink::StepTextSuggester::Context.should_receive(:new).with(@text, @position, @scenario)
    ...
    describe "when the suggestion limit" do
      ...
      describe "is never reached" do
        ...
        it "should execute all suggesters in the chain" do
          Shrink::StepTextSuggester::Suggester::SUGGESTER_CHAIN.each do |suggester|
      ...
      describe "is reached" do
        ...
        it "should not execute suggesters after the suggester that reached the limit allowed" do
          ...
          first_suggester = Shrink::StepTextSuggester::Suggester::SUGGESTER_CHAIN.first
          first_suggester.should_receive(:suggestions_for)

          other_suggesters = Shrink::StepTextSuggester::Suggester::SUGGESTER_CHAIN[1..-1]
    ...
    it "should add the suggestions offered by each suggester to the context" do
      Shrink::StepTextSuggester::Suggester::SUGGESTER_CHAIN.each_with_index do |suggester, i|
    ...
    def suggestions_for
      Shrink::StepTextSuggester::Suggester.suggestions_for(@text, @position, @scenario)
    end
