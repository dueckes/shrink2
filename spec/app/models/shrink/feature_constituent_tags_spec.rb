describe Shrink::FeatureConstituentTags do
  ...
  class TestableFeatureConstituentTags < Array
    ...
    def initialize(proxy_owner=nil)
      @proxy_association = StubProxyAssociation.new(proxy_owner)
    end
  ...
  context "#line" do
    ...
    let(:feature_constituent_tags) { TestableFeatureConstituentTags.new }
  ...
  context "#line=" do
    ...
    let(:project) { Shrink::Project.new(:name => "Some Project") }
    let(:feature) do
      Shrink::Feature.new(:title => "Some Feature", :folder => Shrink::Folder.new(:project => project)) do |feature|
        feature.stub!(:update_summary!)
      end
    end
    let(:feature_constituent_tags) { create_feature_constituent_tags(feature) }
    let(:tag_name) { "Some Tag" }
    let(:tag) { Shrink::Tag.new(:name => tag_name) }

    before(:each) { Shrink::Tag.stub!(:find_or_create!).and_return(tag) }
    ...
    describe "when a tag is removed" do
      ...
      before(:each) { feature_constituent_tags << tag }
    ...
    describe "when multiple tags are added and removed" do
      ...
      let(:tags) do
        create_tags(:number => 6, :project => project) do |tag|
          Shrink::Tag.stub!(:find_or_create!).with(:name => tag.name, :project => project).and_return(tag)
        end
      end
      let(:feature_constituent_tags_to_remove) { tags[0..2] }
      let(:feature_constituent_tags_to_add) { tags[3..5] }
  ...
  context "#unused" do
    ...
    let(:project) do
      Shrink::Project.new(:name => "Some Project") do |project|
        project.stub!(:tags).and_return(project_tags)
      end
    end
    let(:project_tags) { mock("ProjectTags") }
    let(:feature) { Shrink::Feature.new(:title => "Some Feature", :folder => Shrink::Folder.new(:project => project)) }
    let(:all_tags) { create_tags(:number => 3) }
    let(:feature_constituent_tags) { create_feature_constituent_tags(feature) }

    it "should return unused tags in the project of the folder of the constituents feature ordered by name" do
      project_tags.should_receive(:all).with(:order => "name").and_return(all_tags)
    ...
    describe "when no tags exist" do
      ...
      before(:each) { project_tags.stub!(:all).and_return([]) }
    ...
    describe "when tags exist" do
      ...
      before(:each) { project_tags.stub!(:all).and_return(all_tags) }
      ...
      describe "and the feature constituent is associated with a tag" do
        ...
        before(:each) { feature_constituent_tags << all_tags[0] }
  ...
  context "#project=" do
    ...
    let(:project) { Shrink::Project.new(:name => "Some Project") }
    let(:feature) { Shrink::Feature.new(:title => "Some Feature") }
    let(:feature_constituent_tags) { create_feature_constituent_tags(feature) }
    ...
    describe "when the feature constituent is associated with many tags" do
      ...
      describe "and the tags are not associated with a project" do
        ...
        let(:tags) do
          create_tags { |tag| feature_constituent_tags << tag }
        end

        it "should associate the tags with the provided project by finding or instantiating a tag in the project with the same name" do
      ...
      describe "and the tags are associated with a project" do
        ...
        let(:tags) do
          create_tags(:project => project) { |tag| feature_constituent_tags << tag }
        end
