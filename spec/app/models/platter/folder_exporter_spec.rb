describe Platter::FolderExporter do

  context "#export" do

    before(:each) do
      @feature_adapter = mock("FeatureAdapter")
      @folder_exporter = Platter::FolderExporter.new(@feature_adapter)
      @folder = mock("Folder")

      @zip_exporter = mock("ZipExporter", :null_object => true)
      Platter::FolderExporter::ZipExporter.stub!(:new).and_return(@zip_exporter)
    end

    it "should perform the export via a ZipExporter" do
      Platter::FolderExporter::ZipExporter.should_receive(:new).with(hash_including(
